...
const AppliedUserProvider = () => {
  ...
  const appliedbycompany = useSelector(state => state.appliedUser.appliedUsers)
  ...
  useEffect(() => {
    if (appliedbycompany !== null || appliedbycompany !== undefined || appliedbycompany.length !== 0) {
      dispatch(getCompanybyAppliedUser('70'))
    }
  }, [dispatch])
  ...
  useEffect(() => {
    if (appliedbycompany) {
      setRecords(appliedbycompany?.slice(firstIndex, lastIndex));
      setPage(Math.ceil(appliedbycompany.length / numbersPerPage));
    }
  }, [appliedbycompany, firstIndex])
