...
import { getCompanybyAppliedUser } from "../store/actions/appliedUserActions";
...
const AppliedUserProvider = () => {
  ...
  const appliedbycompany = useSelector(
    (state) => state.appliedUser.appliedUsers
  );
  const loading = useSelector((state) => state.appliedUser.isLoading);
  const providerId = sessionStorage.getItem(SESSION_PROVIDER_ID);

  useEffect(() => {
    console.log("applied", appliedbycompany);
  }, [appliedbycompany]);

  useEffect(() => {
    if (
      appliedbycompany !== null ||
      appliedbycompany !== undefined ||
      appliedbycompany.length !== 0
    ) {
      dispatch(getCompanybyAppliedUser(providerId));
    }
  }, [dispatch, providerId]);
  ...
  useEffect(() => {
    if (appliedbycompany) {
      setPaginatedData(appliedbycompany?.slice(startIndex, endIndex));
    }
  }, [appliedbycompany, startIndex]);

  // search===============
  const [search, setSearch] = useState("");
  useEffect(() => {
    const result = appliedbycompany?.filter((item) => {
    ...
    <PortalLayout>
      {loading ? (
        ...
        <>
          {nodata ? (
            <>
              <h1 className=" text-[2rem] text-gray-500 ">No User Found</h1>
            </>
          ) : (
            <>
              ...
              <div className="w-[100%] max-md:h-full  max-md:px-2 flex flex-col justify-center bg-white">
                ...
                <ThemeProvider theme={theme}>
                  <Stack direction="row" justifyContent="center" marginTop={2}>
                    <Pagination
                      count={Math.ceil(appliedbycompany?.length / itemsPerPage)}
                      page={currentPage}
...
export default AppliedUserProvider;
