...
const Offers = () => {
  ...
  useEffect(() => {
    if (offerByCompany) {
      setPaginatedData(offerByCompany?.slice(startIndex, endIndex));
    }
  }, [offerByCompany, startIndex]);

  // search===============
  const [search, setSearch] = useState("");
  useEffect(() => {
    const result = offerByCompany?.filter((item) => {
      return item?.offerType?.toLowerCase()?.match(search?.toLocaleLowerCase());
    });
              ...
              <div className="w-[100%] max-md:h-full  max-md:px-2 flex flex-col justify-center bg-white">
                ...
                <ThemeProvider theme={theme}>
                  <Stack direction="row" justifyContent="center" marginTop={2}>
                    <Pagination
                      count={Math.ceil(offerByCompany?.length / itemsPerPage)}
                      page={currentPage}
