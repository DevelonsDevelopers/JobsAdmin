...
import { AllOffers, GetOffersByCompany } from '../store/actions/offersActions';
import OfferView from './view/Offers';
...
const Offers = () => {
  ...
  useEffect(() => {
    if (offerByCompany) {
      setRecords(offerByCompany.slice(firstIndex, lastIndex));
      setPage(Math.ceil(offerByCompany.length / numbersPerPage));
    }
  }, [offerByCompany, firstIndex])
    ...
    <PortalLayout>
      {loading ? <center> <div className="flex justify-center items-center h-screen">
        ...
        <>
          {nodata ?
           <>
           <h1 className=" text-[2rem] text-gray-500 " >No Offers Yet</h1>
         </>
            :
            <>
              ...
              <div className="w-[100%] max-md:h-full  max-md:px-2 flex flex-col justify-center bg-gray-100">
                ...
                <OfferView open={openView} setOpen={setOpenView} title={"VIEW"} data={data} ID={viewId} />
                <table className="rounded-xl p-5 bg-white w-[90%] m-auto max-md:w-[100%]  mt-6 ">
                  <thead className='mt-10'>
                    <tr className=" uppercase  text-sm leading-normal w-[100%]">
                      ...
                      <th className="py-[2%] border-r-[1px] border-b-[2px] border-b-black w-[1%] max-md:text-[.6rem] max-md:font-[400] text-center max-md:w-[3%] text-[13px]">OfferType</th>
